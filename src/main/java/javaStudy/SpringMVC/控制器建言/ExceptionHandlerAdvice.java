...
public class ExceptionHandlerAdvice {
    ...
    // session备份的键值绑定。当不是点对点同名传值时，间接通过Model空间的键值绑定，有两种情况：1.手动添加的键值，直接绑定，
    // 2.自动添加的键值（类型猜测？）一对绑定的类名首字母小写-对象，一对绑定的结果描述，但对象是空对象。最后通过点对点，点对setter
    // 完成最终传参
    //【理解】对于@InitBinder来说 @ModelAttribute和@RequestMapping的地位是相同的，都要根据参数寻求绑定，同名绑定null，不会存
    // 到model空间，对象绑定引用,只不过引用有是否已经有值之分。引用存在Model空间，有的是手动加的，有的则是类型猜测自动加的。
    @InitBinder
