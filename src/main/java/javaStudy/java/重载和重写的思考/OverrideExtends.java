...
/**
 * 重写是子类对父类的允许访问的方法的实现过程进行重新编写,发生在子类中，方法名、参数列表必须相同，返回值范围小于等于父类(更具体)，抛出的异常范围
 * 小于等于父类(更具体)，访问修饰符范围大于等于父类（不能私藏，可以分享）。另外，如果父类方法访问修饰符为 private 则子类就不能重写该方法。也就
 * 是说方法提供的行为改变，而方法的外貌并没有改变。
 *
 * @author dhslegen
 */
